...
function step(g, m, n) {
  ...
  for(let i = m; i <= n; i++) {
    ...
    //Find step & return it
  for(let w = 0; w < primes.length; w++) {
    for(let k = w; k < primes.length; k++) {
      if(primes[k] - primes[w] === g) {
        return [ primes[w], primes[k] ];
      }
    }
  }
}
...
//Tests
step(2,100,110); //should return [101, 103]
step(4,100,110); //should return [103, 107]
step(6,100,110); //should return [101, 107]
step(8,300,400); //should return [359, 367]
step(10,300,400); //should return [307, 317]
